...
local function showChangelog()
    PerformHttpRequest(GITHUB_CHANGELOG, function(status, body)
        if status == 200 and body and body ~= "" then
            ...
            print("^1[fenix-passwordsmanager]^0 Failed to retrieve remote changelog.")
        end
    end, "GET")
end
...
CreateThread(function()
    ...
    PerformHttpRequest(GITHUB_VERSION, function(status, body)
        ...
        if status ~= 200 or not body then
            print("^1[fenix-passwordsmanager]^0 Failed to check for updates on GitHub.")
            return
        end
        ...
        if not latest then
            print("^1[fenix-passwordsmanager]^0 Remote version file is invalid.")
            return
        end

        if latest ~= current then
            print(("^3[fenix-passwordsmanager]^0 New version available: ^2%s^0 (current: ^1%s^0)"):format(latest, current or "unknown"))
            print("^3Download on Github: https://github.com/IamFenixDesign/fenix-passwordsmanager^0")

            -- Only show changelog when version does NOT match
            showChangelog()
        else
            print(("^2[fenix-passwordsmanager]^0 You are running the latest version (%s)"):format(current))
        end
    end, "GET")
end)
